...
Climate change leads to changing environmental conditions for crops. Plants will respond to those changes with shifting biomass partitioning patterns to obtain the most limiting resource. Here we use drought stress as a limiting factor.\
...
We implemented the model in Similie (Version 6.12). We used a simplified growth and LAI model, as the base model, where after LAI reaching a value of 0.7, half of the growth rate (dW) is added to the fraction of the leaves.\
\
$$dLAI=
    \begin{cases}
    r_g\ *\ LAI\ &\   if\ LAI\ <\ 0.7\\ 
...
\
The `Simile` model was implemented in `Simile` version v6.12p9 teaching. 64-bit.\
\
The `R` model was implemented in `R` version 4.2.2 "Innocent and Trusting".\
...
The data for the fractions originally come from the Lintul model and were transferred from there to our model. How the fractions were collected is not clearly documented, but there are several experimental possibilities.
...
>>>>>>> 7e241239c043b137f0426ed5a20a26d8c93d9671
\
